...
class GATNet(torch.nn.Module):
    def __init__(self, n_output=1, num_features_xd=69, num_features_xt=33, latent_dim=128, output_dim=128, heads=1, dropout=0.2):
        super(GATNet, self).__init__()

        # SMILES graph branch
        self.n_output = n_output
        self.dconv1 = GATv2Conv(num_features_xd, num_features_xd, add_self_loops=True, heads=heads, concat=False)
        self.dconv2 = GATv2Conv(num_features_xd, num_features_xd*2, add_self_loops=True, heads=heads, concat=False)
        self.dconv3 = GATv2Conv(num_features_xd*2, num_features_xd*4, add_self_loops=True, heads=heads, concat=False)
        self.fc_gd1 = torch.nn.Linear(num_features_xd*4, 1024)
        self.fc_gd2 = torch.nn.Linear(1024, output_dim)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout)
