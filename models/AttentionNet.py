...
class CrossAttention(nn.Module):
    ...
    def forward(self, cross_x, target_x):

        B, C = cross_x.shape
        q = self.wq(cross_x).reshape(B, self.num_heads, C // self.num_heads)  # B1C -> B1H(C/H) -> BH1(C/H)
        k = self.wk(target_x).reshape(B, self.num_heads, C // self.num_heads) # BNC -> BNH(C/H) -> BHN(C/H)
        v = self.wv(target_x).reshape(B, self.num_heads, C // self.num_heads) # BNC -> BNH(C/H) -> BHN(C/H)

        attn = (q @ k.transpose(-2, -1)) * self.scale  # BH1(C/H) @ BH(C/H)N -> BH1N
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, C)   # (BH1N @ BHN(C/H)) -> BH1(C/H) -> B1H(C/H) -> B1C
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
############################################################################################################
# Note that this attention approach cannot tell any information about "Binding site"
# Because this attend after global pooling which means it attend on feature vector not a position.
# Just stupid cross Attention --> Context vector only effect on Target protein not drug. (First try)
class AttentionNet(nn.Module):
    def __init__(self, n_output=1, num_features_xd=69, num_features_xt=33, latent_dim=128, output_dim=128, dropout=0.2):
        ...
        # Attention layer
        self.d_attention = CrossAttention(output_dim, num_heads=4)
        self.t_attention = CrossAttention(output_dim, num_heads=4)
    ...
    def forward(self, data_mol, data_prot):
        ...
        d_context_vec = self.d_attention(xt, x)
        t_context_vec = self.t_attention(x, xt)
